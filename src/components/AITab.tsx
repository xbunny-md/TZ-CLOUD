...
export default function AITab() {
  ...
  useEffect(() => {
    localStorage.setItem('tz-cloudmind-sessions', JSON.stringify(sessions));
  }, [sessions]);
  ...
  const createNewSession = () => {
    const newSession: ChatSession = {
      ...
      name: 'New Session',
      pinned: false,
      messages: [{ id: '1', role: 'system', content: 'TZ CloudMind initialized. Ready for input.' }],
      createdAt: Date.now()
    };
  ...
  const deleteSession = (id: string) => {
    setSessions(prev => {
      ...
      if (updated.length === 0) {
        return [{ id: Date.now().toString(), name: 'New Session', pinned: false, messages: [{ id: '1', role: 'system', content: 'Hello.' }], createdAt: Date.now() }];
      }
  ...
  const handleSendMessage = async (e?: React.FormEvent, forceInput?: string) => {
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ prompt: userMessage.content, history: activeSession.messages })
      });
      ...
      updateSessionMessages(activeSessionId, [...newMessages, {
        id: (Date.now() + 1).toString(),
        role: 'system',
        content: "Error: Neural core disconnected. Please check your keys or connection."
      }]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="w-full h-[85vh] flex pb-6 relative overflow-hidden">
      ...
      <AnimatePresence>
        {isSidebarOpen && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 bg-black/60 backdrop-blur-sm z-40 lg:hidden rounded-2xl"
      ...
      <div className={`absolute lg:relative w-[85%] max-w-[320px] lg:w-64 lg:mr-6 glass-card rounded-2xl border-white/10 flex flex-col overflow-hidden h-full z-50 transition-transform duration-300 ease-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-[120%] lg:translate-x-0'}`}>
        <div className="p-4 border-b border-white/10 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setIsSidebarOpen(false)} className="p-2 rounded-lg bg-white/5 hover:bg-white/20 transition-colors lg:hidden">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
      ...
      <div className="flex-1 glass-panel rounded-2xl flex flex-col overflow-hidden border-aurora-purple/30 relative h-full w-full">
        ...
        <div className="p-4 lg:p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden lg:flex w-12 h-12 rounded-xl bg-aurora-purple/20 items-center justify-center border border-aurora-purple/50 shadow-[0_0_15px_rgba(138,43,226,0.3)]">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] lg:text-xs font-mono text-white/50 uppercase">Core Active • Groq/Gemini</span>
              </div>
            </div>
          </div>
          <button 
            onClick={() => updateSessionMessages(activeSessionId, [])}
            className="p-2 rounded-lg bg-white/5 hover:bg-red-500/20 text-white/50 hover:text-red-400 transition-colors flex items-center gap-2"
            title="Clear Chat"
          >
            <RefreshCw className="w-4 h-4" />
            <span className="hidden sm:inline text-xs font-mono uppercase tracking-wider">Clear</span>
          </button>
        </div>
